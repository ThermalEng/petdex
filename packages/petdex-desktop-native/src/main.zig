...
const shell_views = [_]native_sdk.ShellView{
    .{ .label = canvas_label, .kind = .gpu_surface, .fill = true, .role = "Pet canvas", .accessibility_label = "Petdex pet", .gpu_backend = if (builtin.target.os.tag == .linux) .software else .metal, .gpu_pixel_format = .bgra8_unorm, .gpu_present_mode = .timer, .gpu_alpha_mode = if (builtin.target.os.tag == .linux) .premultiplied else .@"opaque", .gpu_color_space = .srgb, .gpu_vsync = true },
};
